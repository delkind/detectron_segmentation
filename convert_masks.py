...
def verify_conversion(ctrs, mask):
    print(f"Source contours shape: {ctrs.shape}")
    ...
    ctrs1, _ = cv2.findContours(cv2.cvtColor(mask, cv2.COLOR_BGRA2GRAY), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    print(f"Resulting contours shape: {ctrs1.shape}")
    return not (ctrs.shape != ctrs1.shape or any(map(lambda c: (c[0] != c[1]).any(), zip(ctrs, ctrs1))))
