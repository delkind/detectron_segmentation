...
def generate_qval_data(data):
    ...
    groups = {
        (g, s[0], p): data[(data.gender == g) & (data.strain == s[1])].groupby('region')[to_col_name(data, p)].apply(
            np.array).sort_index()
    ...
    for p in params:
        for g in genders:
            ...
            qvals[f"BL6_vs_CD1_{g}_{p}_effect"] = 2 * (local_data.left.apply(mdn) - local_data.right.apply(mdn)) / (
                        local_data.left.apply(mdn) + local_data.right.apply(mdn))
    ...
    for strain_name, strain in strains.items():
        ...
        all_groups = strain_data.groupby('region')
        for param in params:
            ...
            qvals[f'{strain_name}_male_vs_female_{param}_effect'] = 2 * (
                    qvals[f'{strain_name}_{param}_male_means'] - qvals[f'{strain_name}_{param}_female_means']) / (
                                                                            qvals[f'{strain_name}_{param}_male_means'] +
                                                                            qvals[
                                                                                f'{strain_name}_{param}_female_means'])
        ...
        for gender, gender_groups in {'male': male_groups, 'female': female_groups, "all": all_groups}.items():
            for param in ['count3d', 'density3d', 'volume']:
                ...
                qvals[f'{strain_name}_{gender}_left_vs_right_{param}_effect'] = 2 * (
                        qvals[f'{strain_name}_{param}_{gender}_right_means'] - qvals[
                    f'{strain_name}_{param}_{gender}_left_means']) / (
                                                                                        qvals[
                                                                                            f'{strain_name}_{param}_{gender}_left_means'] +
                                                                                        qvals[
                                                                                            f'{strain_name}_{param}_{gender}_right_means'])
                for method_name, method in {'t-test': scipy.stats.ttest_ind, 'ranksum': scipy.stats.ranksums}.items():
...
def produce_volcano_plot(qvals, strain, prefix, x_col, y_col, filename, hide_rejected=False):
    ...
    full_fname = strain + '_' + prefix + '_' + filename
    print(f'{full_fname}: total significant: {len(ann[0])}, negative: {len(np.where(x[ann] < 0)[0])}, positive: {len(np.where(x[ann] > 0)[0])}')
    plot_grid(ax, horiz_lines=[2], vert_lines=[0.05, -0.05])
    produce_figure(ax, fig, full_fname)
...
def figure3(data):
    ...
    for strain in strains:
        plot_effect_vs_effect(qvals, strain, 'male_vs_female', 'density3d', 'volume',
                              f'density_effect_vs_volume_effect')
        ...
        for param, gender in itertools.product(['count3d', 'density3d', 'volume'], genders + ['all']):
            produce_volcano_plot(qvals, strain, 'left_vs_right',
                                 f'{gender}_left_vs_right_{param}_effect',
                                 f'{param}_{gender}_ranksum',
                                 f"{param}_{gender}_qval_vs_normalized_L-R")

        for (param, highlight_param), gender in itertools.product([('count3d', 'density3d'),
                                                                   ('density3d', 'count3d')],
                                                                  genders + ['all']):
            plot_qval_vs_qval(qvals, strain, 'left_vs_right',
                              f'{highlight_param}_{gender}_ranksum',
                              f'{param}_{gender}_ranksum',
                              f'volume_{gender}_ranksum',
